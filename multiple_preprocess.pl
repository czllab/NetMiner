...
GetOptions(
	...
	"pbspath|pbs=s" => \$pbspath,
	"rawfastqpath|sra=s" => \$rawfastqpath,
	"fastqcpath|fastqc=s" => \$fastqcpath,
	"cleanpath|clean=s" => \$cleanpath,
...
EOF
				...
				if(!-e $flagsample){
					open OUT,">$$outputpath/$$pbspath/$runHash{$run}/$jobname\_$runid".".pbs";
					print OUT <<EOF;
#PBS -N $jobname\_$species\_$runid
#PBS -j oe
#PBS -l nodes=1:ppn=$$threads
#PBS -q $queue
#PBS -o $$outputpath/$$pbspath/$runHash{$run}/$jobname\_$runid.log
...
sub submitJob{
	...
	foreach my $run (keys %$runHash){
		my $taskNum =`qstat -uhyu | grep $jobname\_$species | grep $queue | wc -l`; 
		while($taskNum > 100){
			print "The num of task remaining $taskNum\n";
			sleep 30;
			print `date`;
			$taskNum = `qstat -uhyu | grep $jobname\_$species | grep $queue | wc -l`;
		}
		my $qsub;
		my $runid = $run;
		$runid =~ s/\.sra//;
		...
		if ($$marks{$runid} == 1){
			...
			print "The task $jobname\_$species\_$runid is successfully finished\n";
		}
		my $lc = localtime;
		
		if ($qsub ne ""){
			print "$lc\nThe task of $jobname\_$species\_$runid was submitted in $qsub\n";
		}
	}
}
